...
def web(host: str, port: int, verbose: bool):
    ...
    console.print("[bold cyan]VulnScout Web UI[/]")
    console.print(f"[dim]启动服务:[/] http://{host}:{port}")

    try:
        import uvicorn
    except ImportError:
        error_console.print("[red]需要安装 web 依赖: pip install 'vulnscout[web]'[/]")
        sys.exit(1)

    # Phase 4 实现 Web UI
    error_console.print("[yellow]Web UI 功能开发中，敬请期待 Phase 4[/]")
    sys.exit(0)
